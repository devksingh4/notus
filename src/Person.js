        ...
        if(this.isWithinErrorOfTarget(.01)){
          ...
          if(time > this.move_at){
            ...
            if (this.current_hold == null){
              ...
              if (table != null){
                this.current_hold = [[table[0], table[1]], "table", table[2]]
...
class Population {
    constructor(pop_size, starting_pos, starting_tar, starting_non_compliant, starting_size, airGrid, naviGrid, infp){
        this.pop = [];
        for(let i = 0; i < pop_size; i++){
            this.pop.push(new Person(starting_pos[i][0], starting_pos[i][1], starting_tar[i][0], starting_tar[i][1], starting_non_compliant[i], starting_size[i], airGrid, naviGrid, infp));
        }
    }
    get size(){
